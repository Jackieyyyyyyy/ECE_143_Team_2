...
def cleanup(rawdoc):
    ...
    Started by TSun May5/2019, last edited by TSun May19/2019.

    :rawdoc: string ending in csv
    :return type: csv file in directory
    '''

    import csv
    from datetime import date

    assert isinstance(rawdoc,str), 'Please input a string represents the data file you want to clean up.'
    assert len(rawdoc)>4, 'Invalid input string.'
    assert rawdoc[-4:]=='.csv', 'Please input a csv file'
    ...
    new_injures = []
    new_temperature = []
    with open(rawdoc, encoding="utf8", errors='ignore') as csvfile:
    ...
    with open('modified_'+rawdoc, mode='w',newline ='') as writer_file:
    ...
    print('Data Clean Up Finished!')



cleanup('PakistanSuicideAttacks_raw.csv')